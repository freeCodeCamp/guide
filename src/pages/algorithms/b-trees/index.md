...
1. if the number of keys is too high, greter than t-1 then we move the middle key to the nodes parent. We do this recursively up until root. If the number of keys in the root is too high, then we make the middle key to be the new root of the whole tree and connect it to the the node it was in before.
