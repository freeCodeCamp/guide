...
CPUs are integrated circuits, which are complex digital circuits embedded on a single chip. They include registers, which store single values, and input/output pins. These pins allow them to talk to the rest of the computer. Frequently, CPUs will also contain more than one core, which allows them to multitask. 
...
Gigahertz is not the only determining factor in the actual speed of a processor, as different processors with the same gigahertz speed (also known as clock speed) may perform real-world tasks at different speeds due to using different sets of instructions to perform these tasks. These instruction sets are called CPU architectures. The architecture of a CPU can greatly differ the system design, resulting in different achievable throughput and frequency (gigahertz). Two frequent versions of traditional CPU architecture include, the von Neumann and Harvard architecture systems. The main difference between von Neumann and Harvard archecture is the von Neumann architectue contains a single set bus for data and instructions between the CPU and cache on-chip memory, while the Harvard architecture contains multiple parallel set buses between CPU and cache on-chip memory. 
