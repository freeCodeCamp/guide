...
The `continue` statement makes the loop skip all the statements following after the `continue` statement and jump ahead to the beginning of the next iteration. In a `for` loop, control jumps to the update statement; and in a `while` or `do while` loop, control jumps to the boolean expression/condition.
...
In case of multiple nested loops, the `continue` jumps to the beginning of the next iteration of the <i>smallest enclosing loop</i>. Additionally, you can use labels to choose a specific loop out of the nested set to skip to the next iteration. 
